...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', 1.5);
	...
	set(gca, 'xticklabel', bw_xlabel, 'box', 'off', 'ticklength', [0 0], 'fontsize', 10, 'xtick',1:numbars, 'linewidth', 1.5,'xgrid','off','ygrid','off');
