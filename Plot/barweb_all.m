function handles = barweb_all(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	handles.bars = bar(barvalues, width,'edgecolor','k', 'linewidth', 1.5);
	hold on
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',13,'fontweight','bold');
	end
	...
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 12, 'xtick',1:numgroups, 'linewidth', 1.5,'xgrid','off','ygrid','off');
