function handles = barweb_color_evt(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type,color_evt)

%


% Revised by AJR 2012/06/11 
% Group number matching xtick label
...
% 	handles.bars = bar(barvalues, width,'edgecolor','k', 'linewidth', 2);
    handlesbar=[];
    for nb_group= 1: size(barvalues,2)
        handlesbar(end+1)= bar([nb_group nb_group+1], barvalues(:,nb_group),width,'edgecolor','k','linewidth',1.5,'facecolor',color_evt(nb_group+1, 1:3));
	hold on
    end
    
    handles.bars=handlesbar;
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'EastOutside', 'fontsize',7);
		legend boxoff;
	else
		handles.legend = [];
	end
	...
	ylim([floor(min(ylimit)) ceil(max(ylimit))]);
    
    
    
	xlim([0.5 nb_group+0.5]);
	
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			xdata = get(handles.errors(i),'xdata');
  
			for j = 1  %% Revised by AJR 2012/06/11
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', 8, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',12);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',12);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',12);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 8, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
