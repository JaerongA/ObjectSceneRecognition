function PETH =  Draw_PETH_color(ts_evt, ref_evt, ts_spk, nb_bins, bin_size, Stimulus_color, draw_mode)
...
for trial_run=1:nb_trial
    ...
    trial_stimulus= ts_evt(trial_run, Stimulus);
...
%             lineh=  line([evt_ts evt_ts],[(sum(nb_trial) -(trial_run)) (sum(nb_trial) -(trial_run))+1],'color',[0.4 0.4 0.4]);
            plot(evt_ts, (nb_trial-(trial_run-0.5)),'Marker','o','MarkerSize',3,'MarkerFaceColor',[0.4 0.4 0.4],'MarkerEdgeColor','none');
        elseif  ref_evt == Choice
