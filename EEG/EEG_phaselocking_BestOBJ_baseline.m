%% Created by Jaerong 2018/06/06  for PER & POR ephys analysis
%% Get phase-locking for best OBJ during the baseline period 
%% The program used for generating the output for CR manuscript.


function EEG_phaselocking_BestOBJ_baseline(summary, summary_eeg, summary_header, summary_header_eeg,dataROOT)
...
baseline_second= 1;  %% Duration of the baseline period
...
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ_baseline(CorrectOnly)\1sbeforeOBJ\Theta' date];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ_baseline(CorrectOnly)\1sbeforeOBJ\Beta'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ_baseline(CorrectOnly)\1sbeforeOBJ\LowGamma'];
saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ_baseline(CorrectOnly)\1sbeforeOBJ\HighGamma' date];
...
outputfile= 'EEG_Phaselocking_OBJ(baseline).csv';
fod=fopen(outputfile,'w');
txt_header = 'Key#(Cluster), RatID, Session, Task_session, Task, Regional_pair, TT(EEG), ClusterID, Cell_type,';
fprintf(fod, txt_header);
txt_header = 'MeanFR, Rayleigh_sig(All), Rayleigh_sig(1st), Rayleigh_sig(2nd),RVL(All), RVL(1st), RVL(2nd), RVL(Diff), Spk#(small_half),';
fprintf(fod, txt_header);
txt_header = 'FittingCAT, Polarity, BestFittingOBJ, BestOBJ_CorrR, BestOBJ_Slope';
fprintf(fod, txt_header);
fprintf(fod,'\n');
fclose(fod);
...
for i_s=  1:c_s
    ...
    if  (str2num(summary(i_s).Repetition_OK)== 1) && (str2num(summary(i_s).Quality_ok)== 1) && ~strcmp(summary(i_s).Region,'POR') && ~strcmp(summary(i_s).Fitting_category,'Nofitting') && ~strcmp(summary(i_s).Fitting_category,'NaN')
        
        
%     if  (str2num(summary(i_s).Key)== 471)
        
        
        
    %% revised on 2018/06/06
...
%                 text(-.1,0.6, msg, 'Fontsize',11, 'backgroundcolor',bgcol(pval_sig.mean_diff+1,:));
                ...
                fprintf(fod,',%1.5f, %d, %d, %d, %1.5f, %1.5f, %1.5f, %1.5f,%d',str2num(summary(i_s).Epoch_FR), pval_sig.All, pval_sig.firsthalf, pval_sig.secondhalf, RV.All, RV.firsthalf, RV.secondhalf, RV.diff, nb_spk_small);
