...
%% Save folder
saveROOT= [dataROOT '\Analysis\EEG\Phaselocking(ALL)\Theta'  ];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking(ALL)\LowGamma\'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking(ALL)\Beta\'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking(ALL)\HighGamma\'];
    ...
    if  (str2num(summary(i_s).Epoch_FR) >= 0.5)  && (str2num(summary(i_s).Zero_FR_Proportion) < 0.5) && ~strcmp(summary(i_s).Region,'POR') ...
        ...
        for Target_run= 1:nb_target
            ...
            if  strcmp(summary_eeg(Target_run).Visual_Inspection, '1') &&  strcmp(summary_eeg(Target_run).Power_criteria, '1') ...
                ...
                ts_evt = ts_evt(select.Corr,:);   %% Correct trial only
...
%                         fig_saveROOT= [ Prefix '-' Prefix2];
                ...
                rose(deg2rad(thisPhase),intDEG); hold on;
                [x,y] = pol2cart(deg2rad(phase_mean),max(get(gca,'Ytick')));
                c= compass(x,y);
                set(gca,'View',[-90 90],'YDir','reverse');
                set(c,'linestyle','-','linewidth',1.5,'color','r');
                ...
                RV.All= circ_r(deg2rad(thisPhase'));
                ...
                msg= sprintf('Resultant Vector Length(All) = %1.3f ', RV.All);
                text(-.3,1, msg, 'Fontsize',12);
                ...
                cd(saveROOT);
                save(mat_name,'HistMAT','thisPhase'); clear thisPhase HistMAT
...
% fclose(fod);
                ...
                fprintf(fod,',%1.5f, %1.5f, %d, %1.5f, %1.5f, %1.5f, %1.5f',str2num(summary(i_s).Epoch_FR), rayleigh_z, pval_sig, phase_mean, phase_var, thisKappa, RV.All);
