...
%% This version used for the CR manuscript & revision

function EEG_phaselocking_novelty_half(summary, summary_eeg, summary_header, summary_header_eeg,dataROOT)
...
outputfile= 'EEG_Phaselocking_Novelty_half.csv';
...
txt_header = 'MeanFR, RVL(Familiar_corr), RVL(Novel_corr), RVL(Familiar_1st), RVL(Familiar_2nd), RVL(Familiar_diff), RVL(Novel_1st), RVL(Novel_2nd), RVL(Novel_diff), Spk#(small_half),';
fprintf(fod, txt_header);
...
%     select.Novel_Corr = find((ts_evt(:,Stimulus)==Owl | ts_evt(:,Stimulus)==Phone)& ts_evt(:,Correctness)==1);
                ...
                HistMAT_Familiar = HistMAT(select.Familiar_Corr);
                
                thisPhase.Familiar_Corr_first = cell2mat(HistMAT_Familiar(1:ceil(size(HistMAT_Familiar,2)/2)));
                thisPhase.Familiar_Corr_second = cell2mat(HistMAT_Familiar(ceil(size(HistMAT_Familiar,2)/2)+1:end));
                ...
                HistMAT_Novel = HistMAT(select.Novel_Corr);
                
                thisPhase.Novel_Corr_first = cell2mat(HistMAT_Novel(1:ceil(size(HistMAT_Novel,2)/2)));
                thisPhase.Novel_Corr_second = cell2mat(HistMAT_Novel(ceil(size(HistMAT_Novel,2)/2)+1:end));
                ...
                RV.Familiar_Corr_first = circ_r(deg2rad(thisPhase.Familiar_Corr_first'));
                RV.Familiar_Corr_second = circ_r(deg2rad(thisPhase.Familiar_Corr_second'));
                
                RV.Novel_Corr_first = circ_r(deg2rad(thisPhase.Novel_Corr_first'));
                RV.Novel_Corr_second = circ_r(deg2rad(thisPhase.Novel_Corr_second'));
                
                
                
                RV.Familiar_diff= RV.Familiar_Corr_second - RV.Familiar_Corr_first;
                RV.Novel_diff= RV.Novel_Corr_second - RV.Novel_Corr_first;
...
%                 txt_header = 'MeanFR, RVL(Familiar_corr), RVL(Novel_corr), RVL(Familiar_1st), RVL(Familiar_2nd), RVL(Familiar_diff), RVL(Novel_1st), RVL(Novel_2nd), RVL(Novel_diff), Spk#(small_half),';
               ...
               %% Output file generation
                ...
                fprintf(fod,',%1.3f, %1.4f, %1.4f,%1.4f, %1.4f,%1.4f, %1.4f, %1.4f, %1.4f, %d',str2num(summary(i_s).Epoch_FR), RV.Familiar_Corr, RV.Novel_Corr, RV.Familiar_Corr_first, RV.Familiar_Corr_second, RV.Familiar_diff, RV.Novel_Corr_first, RV.Novel_Corr_second, RV.Novel_diff, nb_spk_small);
