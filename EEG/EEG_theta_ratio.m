...
function [summary,summary_header]=EEG_theta_ratio(summary, summary_header,dataROOT)
...
for i_s= 1:c_s
    ...
    if   strcmp(summary(i_s).Task_name,'OCRS(FourOBJ)')
        ...
        for trial_run= 1:size(ts_evt,1) % for each trial
            ...
            ind.theta= find(F<=freq_range.theta(end) & F>=freq_range.theta(1));
            ind.beta= find(F<=freq_range.beta(end) & F>=freq_range.beta(1));
            ind.lowgamma= find(F<=freq_range.lowgamma(end) & F>=freq_range.lowgamma(1));
            ind.highgamma= find(F<=freq_range.highgamma(end) & F>=freq_range.highgamma(1));
            
            
            power(trial_run).theta = trapz(S_epoch(trial_run,ind.theta));
            power(trial_run).beta = trapz(S_epoch(trial_run,ind.beta));
            power(trial_run).lowgamma = trapz(S_epoch(trial_run,ind.lowgamma));
            power(trial_run).highgamma = trapz(S_epoch(trial_run,ind.highgamma));
            
            
            
            
            
            power_norm(trial_run).theta = trapz(log(S_epoch(trial_run,ind.theta)))/trapz(log(S_baseline(trial_run,ind.theta)));
            power_norm(trial_run).beta = trapz(log(S_epoch(trial_run,ind.beta)))/trapz(log(S_baseline(trial_run,ind.beta)));
            power_norm(trial_run).lowgamma = trapz(log(S_epoch(trial_run,ind.lowgamma)))/trapz(log(S_baseline(trial_run,ind.lowgamma)));
            power_norm(trial_run).highgamma = trapz(log(S_epoch(trial_run,ind.highgamma)))/trapz(log(S_baseline(trial_run,ind.highgamma)));
            
            
            %             plot(f,S(trial_run).epoch./S(trial_run).baseline,'linewidth',1.5);
            %             box off; hold on;
            %             xlabel(['Frequency (Hz)']);
            %             ylabel(['Norm. Power']);
        ...
        %         mean([power(select.Familiar_Corr).theta])
        %         mean([power(select.Novel_Corr).theta])
        %
        %         mean([power(select.Familiar_Corr).lowgamma])
        %         mean([power(select.Novel_Corr).lowgamma])
        %
        %         mean([power(select.Familiar_Corr).highgamma])
        %         mean([power(select.Novel_Corr).highgamma])
        %
        %
        ...
        %
        %     mean([power(select.Familiar_Corr).theta])
        %     mean([power(select.Familiar_Incorr).theta])
        %
        %     mean([power(select.Novel_Corr).theta])
        %     mean([power(select.Novel_Incorr).theta])
        %
        %     mean([power(select.Familiar_Corr).lowgamma])
        %     mean([power(select.Familiar_Incorr).lowgamma])
        %
        %     mean([power(select.Novel_Corr).lowgamma])
        %     mean([power(select.Novel_Incorr).lowgamma])
        %
        %
        %
