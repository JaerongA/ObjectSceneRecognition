...
function [summary,summary_header]=EEG_crosscorr_filtered(summary, summary_header,dataROOT)
...
fig_pos = [400 400 1800 600];
fig_ok= 0;
filteredOK = 0;


%% Save folder
% saveROOT= [dataROOT '\Analysis\EEG\CrossCorr\Filtered\Theta\' date];
% saveROOT= [dataROOT '\Analysis\EEG\CrossCorr\Filtered\Highgamma\' date];
saveROOT= [dataROOT '\Analysis\EEG\CrossCorr\Filtered\Unfiltered\' date];
...
outputfile= 'EEG_CrossCorr(Filtered).csv';

fod=fopen(outputfile,'w');
txt_header = 'Key(Ref), Key(Target), RatID, Session, Task_session, Task, TT(Ref), TT(Target), Region(Ref), Region(Target), RefRegion, RegionalPair,';
fprintf(fod, txt_header);
txt_header = 'meanR, FilterRange';
fprintf(fod, txt_header);
fprintf(fod,'\n');
fclose(fod);
...
%     if  strcmp(summary(i_s).Power_criteria, '1') && strcmp(summary(i_s).Region, 'PER')
%     if  strcmp(summary(i_s).Second_revision, '1') && strcmp(summary(i_s).Region, 'PER')
            if  strcmp(summary(i_s).Power_criteria, '1') && strcmp(summary(i_s).Region, 'PER')
        ...
        for Target_run= 1: nb_target
            ...
            if  strcmp(summary(Target_run).Power_criteria, '1')  && strcmp(summary(Target_run).Region, 'IntHIPP') && strcmp(RatID, RatID2) && strcmp(Session, Session2)
%             if  strcmp(summary(Target_run).Second_revision, '1')  && strcmp(summary(Target_run).Region, 'IntHIPP') && strcmp(RatID, RatID2) && strcmp(Session, Session2)
                ...
                for trial_run= 1:size(ts_evt,1) % for each trial
                    ...
                    % Filter the EEG in the range of our interest
                    
                    if filteredOK
                    
                    thisEEG.REF = eeg_filter(thisEEG.REF,filter_order, Wn, sampling_freq, ftype);
                    thisEEG.TARGET = eeg_filter(thisEEG.TARGET,filter_order, Wn, sampling_freq, ftype);
                    ...
                    coeff = corrcoef(thisEEG.REF,thisEEG.TARGET);
                    meanR(trial_run) = fisherz(coeff(1,2));
                    ...
                    if fig_ok
                        ...
                        subplot('Position', [0.1 0.98 0.3 0.2]);
                        ...
                        ylabel('Filtered EEG','fontsize',15);
                        xlabel('Time(s)');
                        ...
                        msg= sprintf('Corr Coef = %1.3f ', meanR(trial_run));
                        text(-.3,0.9, msg, 'Fontsize',12);
                ...
                fig_pos= [400 400 1000 500];
                
                
                fig=figure('name',fig_name(1:end-4),'Color',[1 1 1],'Position',fig_pos);
                
                subplot('Position', [0 0.98 0.3 0.2]);
                ...
                msg= sprintf('Leading Region = %s ', Leading_region);
                text(-.3,0.9, msg, 'Fontsize',12);
                ...
                % fod=fopen(outputfile,'w');
                % txt_header = 'Key(Ref), Key(Target), RatID, Session, Task_session, Task, TT(PER), TT(HIPP), Region(Ref), Region(Target), RegionalPair';
                % fprintf(fod, txt_header);
                % txt_header = 'maxR,';
                ...
                fprintf(fod,'%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s',Key, Key2, RatID, Session, Task_session,  Task, TTID, TTID2, Region, Region2, RegionalPair, Ref_Region);
                fprintf(fod,',%1.5f,',nanmean(meanR));
                fprintf(fod,'\n');
                fclose('all');
