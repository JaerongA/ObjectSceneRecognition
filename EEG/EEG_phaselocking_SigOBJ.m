%% Created by Jaerong 2017/10/17  for PER & POR ephys analysis
%% Examines the phase relationship for all objects that showed significant linear fitting within a session.
%% Used Spline smoothing methods


function EEG_phaselocking_SigOBJ(summary, summary_eeg, summary_header, summary_header_eeg,dataROOT)
...
%% Save folder
saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_SigOBJ(CorrectOnly)\Theta\' ];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_SigOBJ(CorrectOnly)\Beta\'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_SigOBJ(CorrectOnly)\LowGamma\'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_SigOBJ(CorrectOnly)\HighGamma\'];
...
outputfile= 'EEG_Phaselocking_SigOBJ.csv';
    ...
    if  (str2num(summary(i_s).Epoch_FR) >= 0.5)  && (str2num(summary(i_s).Zero_FR_Proportion) < 0.5) && ~strcmp(summary(i_s).Region,'POR') ...
        ...
        for Target_run= 1:nb_target
            ...
            if  strcmp(summary_eeg(Target_run).Visual_Inspection, '1') &&  strcmp(summary_eeg(Target_run).Power_criteria, '1') ...
                ...
                SigOBJ = str2num(summary(i_s).SigOBJ);
                ...
                switch SigOBJ
                    ...
                    case 0011
                        ts_evt(sort([select.Stimulus3_Corr ; select.Stimulus4_Corr]),:);
                    case 0100
                        ts_evt = ts_evt(select.Stimulus2_Corr,:);
                    case 0101
                        ts_evt = ts_evt(sort([select.Stimulus2_Corr ; select.Stimulus4_Corr]),:);
                    case 0110
                        ts_evt = ts_evt(sort([select.Stimulus2_Corr ; select.Stimulus3_Corr]),:);
                    case 0111
                        ts_evt = ts_evt(sort([select.Stimulus2_Corr ; select.Stimulus3_Corr ; select.Stimulus4_Corr]),:);
                    case 1000
                        ts_evt = ts_evt(select.Stimulus1_Corr,:);
                    case 1001
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus4_Corr]),:);
                    case 1010
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus3_Corr]),:);
                    case 1011
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus3_Corr ; select.Stimulus4_Corr]),:);
                    case 1100
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus2_Corr]),:);
                    case 1101
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus2_Corr ; select.Stimulus4_Corr]),:);
                    case 1110
                        ts_evt = ts_evt(sort([select.Stimulus1_Corr ; select.Stimulus2_Corr ; select.Stimulus3_Corr]),:);
                    case 1111
...
% %                 set(P, 'Visible', 'off'); hold on;
                ...
                msg= sprintf('Rayleigh test Z (1st) = %1.2f', rayleigh_z.firsthalf);
                text(-.1,.2, msg, 'Fontsize',11);
                
                msg= sprintf('Rayleigh P (1st) = %1.2f', rayleigh_pval.firsthalf);
                ...
                msg= sprintf('Rayleigh P (2nd) = %1.2f', rayleigh_pval.secondhalf);
                ...
                msg= sprintf('WW (mean diff) Pval = %1.2f',phase_mean_diff_pval);
                ...
                fprintf(fod,',%1.5f, %d, %d, %d, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %d, %d',str2num(summary(i_s).Epoch_FR), pval_sig.All, pval_sig.firsthalf, pval_sig.secondhalf, phase_mean.All, phase_mean.firsthalf, phase_mean.secondhalf, RV.All, RV.firsthalf, RV.secondhalf, pval_sig.mean_diff, nb_spk_small);
                fprintf(fod,',%s, %s, %s',summary(i_s).Fitting_category, summary(i_s).Polarity, BestFittingOBJ);
                fprintf(fod,'\n');
                fclose('all');
