%% Created by Jaerong 2017/10/13  for PER & POR ephys analysis
%% Get phase-locking OBJ (2017/10/13) using the best-fitting object only
%% The program used for generating the output for CR manuscript.


function EEG_phaselocking_BestOBJ(summary, summary_eeg, summary_header, summary_header_eeg,dataROOT)
...
Repetition_OK = 1;  % (1 for units that are ok to use for repetition analysis (smaller half spike >= 30);
...
%% Save folder (Correct trials only)
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(CorrectOnly)\Theta' date  ];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(CorrectOnly)\Beta' date];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(CorrectOnly)\LowGamma' date];
saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(CorrectOnly)\HighGamma' date];



%% Save folder  (Including the ALL trials)
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(ALL)\Theta' date];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(ALL)\Beta'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(ALL)\LowGamma'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(ALL)\HighGamma' date];



%% Save folder  (Including the error trials)
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(Error)\Theta' ];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(Error)\Beta'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(Error)\LowGamma'];
% saveROOT= [dataROOT '\Analysis\EEG\Phaselocking_BestOBJ(Error)\HighGamma'];
...
txt_header = 'Key#(Cluster), RatID, Session, Task_session, Task, Regional_pair, Layer_pair(spk-eeg), TT(EEG), ClusterID, Cell_type, RefRegion,';
fprintf(fod, txt_header);
txt_header = 'MeanFR, Rayleigh_sig(All), Rayleigh_sig(1st), Rayleigh_sig(2nd), Phase_mean(All), Phase_mean(1st), Phase_mean(2nd), RVL(All), RVL(1st), RVL(2nd), RVL(Diff), Phase_mean_diff_sig, Spk#(small_half),';
fprintf(fod, txt_header);
txt_header = 'FittingCAT, Polarity, BestFittingOBJ, BestFittingOBJ(Perf), BestOBJ_CorrR, BestOBJ_Slope';
fprintf(fod, txt_header);
fprintf(fod,'\n');
fclose(fod);
...
% if  strcmp(summary(i_s).Key,'620')
        
        
%     if  (strcmp(summary(i_s).Repetition_OK,'1')) && (strcmp(summary(i_s).Quality_ok, '1')) && ~strcmp(summary(i_s).Region,'POR') && ~strcmp(summary(i_s).Fitting_category,'Nofitting') && ~strcmp(summary(i_s).Fitting_category,'NaN')



    if  (strcmp(summary(i_s).Quality_ok, '1')) && ~strcmp(summary(i_s).Fitting_category,'Nofitting') && ~strcmp(summary(i_s).Fitting_category,'NaN') && strcmp(summary(i_s).Repetition_OK,'1')
%     if  (strcmp(summary(i_s).Key, '869')) && (strcmp(summary(i_s).Quality_ok, '1')) && ~strcmp(summary(i_s).Fitting_category,'Nofitting') && ~strcmp(summary(i_s).Fitting_category,'NaN')
        

        
        %% revised on 2018/05/30
...
%                 All (including error trials)
%                 
%                 switch find(strcmp(Stimulus_str, BestFittingOBJ))
%                     case 1
%                         ts_evt = ts_evt(select.Stimulus1_All,:);
%                     case 2
%                         ts_evt = ts_evt(select.Stimulus2_All,:);
%                     case 3
%                         ts_evt = ts_evt(select.Stimulus3_All,:);
%                     case 4
%                         ts_evt = ts_evt(select.Stimulus4_All,:);
%                 end
% 
%                 
%                 
...
%                 
                BestFitting_Perf=[];
                BestFitting_Perf= mean(ts_evt(:,Correctness));
...
%                 switch find(strcmp(Stimulus_str, BestFittingOBJ))
%                     case 1
%                         ts_evt = ts_evt(select.Stimulus1_Incorr,:);
%                     case 2
%                         ts_evt = ts_evt(select.Stimulus2_Incorr,:);
%                     case 3
%                         ts_evt = ts_evt(select.Stimulus3_Incorr,:);
%                     case 4
%                         ts_evt = ts_evt(select.Stimulus4_Incorr,:);
%                 end
                ...
                bit2microvolt = str2num(summary_eeg(Target_run).ADBitmicrovolts);
                ...
                for trial_run= 1:size(ts_evt,1) % for each trial
                    ...
                    [thisEEG,thisEEGTS] = get_EEG(csc_ID, ts_vector, bit2microvolt);
...
%                         plot(((thisEEGTS-thisEEGTS(1))), thisEEG,'Color',[.3 .3 .3]); hold on;
                ...
                thisPhase.All= cell2mat(HistMAT); if isempty(thisPhase.All); continue; end
...
%                 phase_mean_diff_pval = circ_kuipertest(deg2rad(thisPhase.firsthalf), deg2rad(thisPhase.secondhalf),intDEG);
                ...
                if nb_spk_small < Spk_criteria
                    ...
                    Repetition_OK = 0;
                    summary_eeg(Target_run).Repetition_OK= sprintf('%d',Repetition_OK);
%                     
                    continue;
...
%                 set(P, 'Visible', 'off'); hold on;
                ...
                msg= sprintf('BestFitOBJ Perf = %2.2f ', BestFitting_Perf*100);
                text(-.1,-.35, msg, 'Fontsize',11);
                ...
                msg= sprintf('RV diff (2nd - 1st) = %1.4f', RV.diff);   %% added 2018/05/28
                ...
                Layer_pair= ['Cluster-' Layer ' vs. EEG-' Layer2];
...
% txt_header = 'Key#(Cluster), RatID, Session, Task_session, Task, Regional_pair, Layer(cluster), TT(EEG), ClusterID, Layer(TT), Cell_type,';
                ...
                fprintf(fod,'%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s ,%s',Key, RatID, Session, Task_session,  Task, Regional_pair, Layer_pair, TTID2, ClusterID, summary(i_s).Cell_type, Ref_Region2);
                ...
                fprintf(fod,',%s, %s, %s, %1.5f, %1.5f, %1.5f,',summary(i_s).Fitting_category, summary(i_s).Polarity, BestFittingOBJ, BestFitting_Perf, str2num(summary(i_s).BestOBJ_CorrR), str2num(summary(i_s).BestOBJ_Slope));
                fprintf(fod,'\n');
                fclose('all');
